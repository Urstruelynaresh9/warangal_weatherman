...
# Dynamically sanitizes the token to prevent "botbot" 404 URL errors on Render
CLEAN_TOKEN = TOKEN if TOKEN.startswith("bot") else f"bot{TOKEN}"
URL = f"https://api.telegram.org/{CLEAN_TOKEN}/"

# Integrated Location to Station ID mapping from your local script
STATION_MAP = {
...
def get_weather_by_location(location_name):
    ...
    # Substring fallback match check
    if not station_id:
        for loc_key, station in STATION_MAP.items():
    ...
    if station_id:
        ...
        # Reduced list serialization to 20 elements to keep Telegram payload sizes within limit bounds
    ...
    while True:
        try:
            ...
            for update in data["result"]:
                ...
                # Unified Auto reply logic checking for greetings before location processing
                if text_lower == "hi":
                    reply = "Hellooooo"
                elif text_lower in ["w", "weather update"]:
                    reply = get_weather_data()  # Default to Bornapalli
                elif text_lower == "list":
                    locations_list = "\n".join([f"• {loc}" for loc in sorted(STATION_MAP.keys())[:20]])
                    reply = f"📍 Available Weather Stations:\n\n{locations_list}\n\nSend any location or village name to get current weather!"
                else:
                    reply = get_weather_by_location(text)

                print(f"📤 SENDING REPLY to (ID: {chat_id}): {reply.splitlines()[0]}")

                # Send reply payload back to the channel
                requests.get(
                    URL + "sendMessage",
                    params={
                        "chat_id": chat_id,
...
@app.route("/")
def home():
    return "Warangal Weather Bot is Natively Merged and Running!"
...
if __name__ == "__main__":
    ...
    # Run loop execution hook for Render port mapping binds
